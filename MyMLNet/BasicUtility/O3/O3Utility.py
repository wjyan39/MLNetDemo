...
def get_clebsh_gordan_coefficient(j1, j2, j, m1, m2, m):
    """
...
class CGExpansion(torch.nn.Module):
    """
    Inverse to tensor product contractions, used to expand a single irrep
    to (2l_1 + 1) x (2l_2 + 1) matrix that represents its contribution to 
    the direct sum representation of the tensor product of two irreps of 
    degree l1 and l2. 
    """
    def __init__(self, metadata:torch.LongTensor, max_l1:int, max_l2:int, overlap_out=True):
        assert metadata.dim() == 1
        assert metadata.minimum().item() > 0
        super().__init__()
        max_l_in = metadata.shape[0]
        self.max_l = min(max_l_in, max_l1 + max_l2) 
        self.max_l1 = max_l_in
        self.max_l2 = max_l2 
    ...
    def _init_params(self, overlap_out:bool):
        ...
        for l in range(self.max_l + 1):
            # loop over l1
            for l1 in range(self.max_l1 + 1):
                # loop over l2 
                for l2 in range(self.max_l2 + 1):
                    # check if current (l1, l2) yields valid l_out
                    if l1 + l2 < l or abs(l1 - l2) > l:
                        continue 
                    # l --> l1 \osum l2
                    cg_source = get_rsh_cg_coefficients(l1, l2, l) 
                    cg_segment = cg_source.repeat_interleave(degeneracy, dim=1)
                    ns_segment = torch.arange(degeneracy).repeat(cg_source.shape[1])
